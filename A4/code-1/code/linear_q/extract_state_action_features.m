...
if nnz(change_grid) > 0 % True, except in initial time-step
    [next_head_loc_m, next_head_loc_n] = find(change_grid > 0);
else % True only in initial time-step
    next_head_loc_m = round(N / 2);
    next_head_loc_n = round(N / 2);
end
head_loc = [next_head_loc_m, next_head_loc_n];
...
if prev_head_loc_m == next_head_loc_m + 1 && prev_head_loc_n == next_head_loc_n     % NORTH
    movement_dir = 1;
elseif prev_head_loc_m == next_head_loc_m && prev_head_loc_n == next_head_loc_n - 1 % EAST
    movement_dir = 2;
elseif prev_head_loc_m == next_head_loc_m - 1 && prev_head_loc_n == next_head_loc_n % SOUTH
    movement_dir = 3;
else                                                                      % WEST
    movement_dir = 4;
end
...
[line,column] = find(grid == -1);
apple_loc = [line,column];
