function [pred, z_test] = cifar10_starter()
    addpath(genpath('./'));
    ...
    net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', 0.1*randn(5,5,16,32)/sqrt(5*5*16/2), 'biases', zeros(32,1)),...
        'padding', [2 2]);
    ...
    net.layers{end+1} = struct('type', 'convolution',...
        'params', struct('weights', 0.1*randn(3,3,32,32)/sqrt(3*3*32/2), 'biases', zeros(32,1)),...
        'padding', [1 1]);
    ...
    save('models/cifar10_baseline_modif.mat', 'net');
